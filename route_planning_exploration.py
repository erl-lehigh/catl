...
def extract_propositions(ts, ast):
    ...
    # print(formula_propositions)
    # print(ts_propositions)
...
def add_reward_exploration_obj(m, ts, time_bound, belief_resources, prob_map, alpha, exp_weights, agent_classes, agents):
    '''Adds the reward constraints and sets the objective function to maximize rewards.
    ...
    - The agent classes given as a dictionary from frozen sets of capabilities
    to bitmaps (integers).
    - Rewards for each location.
    - visited map for each location.
    - alpha for exploration.
    - Time bound.
    '''
    # Define additional binary variables y_x for each location
    y = {}
    for u in ts.g.nodes():
        y[u] = m.addVar(vtype=GRB.BINARY, name='y_{u}'.format(u=u))

    n_agents = defaultdict(int)
    for _, capabilities in agents:
        n_agents[frozenset(capabilities)] += 1

    n_agents = dict(n_agents)
    for u, _ in ts.g.nodes(data=True):
        for k in range(time_bound):
            for g in agent_classes:
                m.addConstr(y[u]*n_agents[g] >= ts.g.node[u]['vars'][k][g])
                # m.addConstr(y[u] >= ts.g.node[u]['vars'][k][g])

    for u, _ in ts.g.nodes(data=True):
        m.addConstr(sum(ts.g.node[u]['vars'][k][g] for k in range(time_bound) for g in agent_classes) >= y[u])

    # reward_weighted = reward_function(belief_resources, exp_weights)
    # print(reward_weighted)
    # Set objective
    # reward_obj = sum(y[u] * reward_weighted[u] for u in ts.g.nodes())/(ts.g.number_of_nodes())

    exploration_obj = sum(y[u] * (prob_map[u]) * alpha for u in ts.g.nodes())

    # exploration_obj = sum(y[u] * skewed_binary_entropy(prob_map[u]) * alpha for u in ts.g.nodes())/ts.g.number_of_nodes()
    # m.setObjectiveN(reward_obj, 3, weight=0.001, name='reward')
    m.setObjectiveN(exploration_obj, 3, weight=0.001, name='exploration')


def add_edge_exploration(m, ts, time_bound, prob_map_edge, alpha, agent_classes):
    ''' similar to node exploration'''
    y_edge = {}
    for u, v, _ in ts.g.edges(data=True):
        if u != v:
            y_edge[(u, v)] = m.addVar(vtype=GRB.BINARY, name='y_{u}_{v}'.format(u=u, v=v))
    for u, v, _ in ts.g.edges(data=True):
        if u != v:
            for k in range(time_bound):
                for g in agent_classes:
                    m.addConstr(y_edge[(u, v)] >= ts.g.edge[u][v]['vars'][k][g])
    for u, v, _ in ts.g.edges(data=True):
        if u != v:
            m.addConstr(sum(ts.g.edge[u][v]['vars'][k][g] for k in range(time_bound) for g in agent_classes) >= y_edge[(u, v)])
    exploration_edge_obj = sum(
        y_edge[(u, v)] * prob_map_edge[(u, v)]
        for u, v, _ in ts.g.edges(data=True)
        if u != v
    )
    m.setObjectiveN(exploration_edge_obj*1e-1*alpha, 4, weight=0.01, name='exploration_edge')

# def reward_function(resources, weights=None):
#     """
#     Calculate the weighted reward for each location using provided weights or default weights of 1.
#
#     :param resources: Dictionary of resources, each containing a dictionary of location-value pairs
#                       e.g., {'r1': {'q1': 6., 'q5': 6.}, 'r2': {'q2': 6., 'q6': 6.}}
#     :param weights: Optional dictionary of weights for each resource type
#                     e.g., {'r1': 0.5, 'r2': 0.3}. If None, uses 1 for all resource types.
#     :return: Dictionary of weighted rewards per location
#              e.g., {'q1': 3.0, 'q2': 1.8, 'q5': 3.0, 'q6': 1.8}
#     """
#     # If weights are not provided, use default weight of 1 for each resource type
#     if weights is None:
#         weights = {r: 1 for r in resources}
#     # Initialize the weighted reward dictionary
#     weighted_reward = {}
#     # Calculate weighted rewards for each location
#     for resource_type, locations in resources.items():
#         weight = weights.get(resource_type, 1)  # Use 1 as default if weight not specified
#         for location, value in locations.items():
#             if location not in weighted_reward:
#                 weighted_reward[location] = 0
#             weighted_reward[location] += value * weight
...
def transportationObjective(m, ts, tra_weight, res_weight, time_bound, 
    ...
    resources_bound = max(sum(resource_distribution[h].get(u, 0) for u in ts.g.nodes())
                         for h in resource_distribution)
    resources_bound = 1 if resources_bound == 0 else resources_bound
...
def route_planning(ts, agents, formula, belief_resources, prob_map, alpha, exp_weights,  prob_map_edge, time_bound=None, variable_bound=None,
    ...
    add_reward_exploration_obj(m, ts, time_bound, belief_resources, prob_map, alpha, exp_weights, agent_classes, agents)
    add_edge_exploration(m, ts, time_bound, prob_map_edge, alpha, agent_classes)
    # run optimizer
